...
class FilteredTreeDataset(InMemoryDataset): # pylint: disable=abstract-method
    ...
    def load_data(self):
        """Load and process tree data from pickle files"""
        data_list = []
        failed_files = []
        accepted_count = 0
        rejected_count = 0
        ...
        for pkl_file in tqdm(root_path.rglob('*.pkl'), total=total_files):
            try:
                graph_data = self._process_pickle_file(pkl_file)
                data_list.append(graph_data)
                accepted_count += 1
            except ValueError as e:
                if str(e) == "Tree too complex":
                    rejected_count += 1
                else:
                    self._handle_file_error(pkl_file, e, failed_files)
        ...
        try:
            ...
            logger.error("Failed to collate data: %s", str(e))
            raise
